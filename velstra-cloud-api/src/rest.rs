...
use crate::{
    ...
    core::{Api, WatchEvent, created_body},
...
async fn read(
    ...
    let _ = headers;
    match target(&path)? {
        Target::Object(name) => Ok(object(StatusCode::OK, api.get(&name).await?)),
        Target::Verb { name, verb } if verb == "explainPlacement" => {
            Ok(Json(api.explain_placement(&name).await?).into_response())
        }
        Target::Verb { name, verb } if verb == "explainMigration" => {
            Ok(Json(api.explain_migration(&name).await?).into_response())
        }
        ...
        Target::Collection { parent, kind } => {
            if query.get("watch").map(|w| w == "true").unwrap_or(false) {
                return watch(api, &parent, &kind, query.get("fromRevision"));
            }
            let listing = api.list(&parent, &kind).await?;
            let body = json!({
...
async fn patch(
    ...
    let updated = api
        .patch(&name, &document(&body)?, if_match(&headers)?)
        .await?;
...
async fn delete(
    ...
    let deleted = api.delete(&name, if_match(&headers)?).await?;
...
fn watch(api: Api, parent: &str, kind: &str, from: Option<&String>) -> ApiResult<Response> {
    let from = from
        .map(|r| {
        ...
        .transpose()?;
    let stream = api.watch(parent, kind, from)?.map(|event| {
        let data = match event {
            WatchEvent::Put(resource) => json!({ "type": "PUT", "resource": to_wire(resource) }),
