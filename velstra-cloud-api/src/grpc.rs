...
use velstra_cloud_model::{
    ...
    resources::{
        ...
        SubnetStatus, VolumeSpec, VolumeStatus,
    },
};
...
impl Service {
    ...
    async fn read<S, T>(&self, name: &str) -> ApiResult<Resource<S, T>>
    where
        S: DeserializeOwned,
        T: DeserializeOwned,
    {
        let name = ResourceName::parse(name)?;
        let document = self.api.get(&name).await?;
        ...
        impl $trait for Service {
            $(
                ...
                async fn $get(&self, request: Request<GetRequest>) -> Result<Response<v1::$message>, Status> {
                    self.who(&request).await?;
                    let resource: Resource<$spec, $status> = self.read(&request.into_inner().name).await?;
                ...
                async fn $list(&self, request: Request<ListRequest>) -> Result<Response<v1::$list_response>, Status> {
                    self.who(&request).await?;
                    let listing = self.api.list(&request.into_inner().parent, $kind).await?;
                    let items = listing
                        .items
                        .into_iter()
                        .map(|document| {
                ...
                async fn $update(
                    ...
                    let updated = self.api.patch(&name, &body, expect(&request.revision)?).await?;
                ...
                async fn $delete(&self, request: Request<DeleteRequest>) -> Result<Response<v1::$message>, Status> {
                    ...
                    let deleted = self.api.delete(&name, expect(&request.revision)?).await?;
                ...
                async fn $watch(&self, request: Request<WatchRequest>) -> Result<Response<Self::$stream>, Status> {
                    ...
                    let stream = self.api.watch(&request.parent, $kind, from)?.filter_map(|event| async move {
                        match event {
...
service! {
    ...
    extra: {
        async fn explain_migration(
            ...
            let answer = self.api.explain_migration(&name).await?;
        ...
        async fn explain_placement(
            ...
            self.who(&request).await?;
            let name = ResourceName::parse(&request.into_inner().name).map_err(ApiError::from)?;
            let answer = self.api.explain_placement(&name).await?;
...
impl Operations for Service {
    ...
    async fn get_operation(
        ...
        self.who(&request).await?;
        let operation: Operation = self.read(&request.into_inner().name).await?;
        Ok(Response::new(v1::Operation::from(&operation)))
    }
    ...
    async fn list_operations(
        ...
        let listing = self
            .api
            .list(&request.into_inner().parent, "operations")
            .await?;
    ...
    async fn watch_operations(
        ...
        let stream = self
            .api
            .watch(&request.parent, "operations", from)?
    ...
    async fn wait_operation(
        ...
        let document = self
            .api
            .wait_operation(&name, Duration::from_millis(request.timeout_millis))
            .await?;
