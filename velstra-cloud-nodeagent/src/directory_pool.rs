...
impl DirectoryPool {
    ...
    /// Copy one file in this pool out to `path`, as qcow2.
    async fn copy_file_out(&self, from: &std::path::Path, what: &str, path: &str) -> Result<u64> {
        if !from.exists() {
            return Err(HostError::failed(format!(
                "{what} is not in this pool, so there is nothing to copy out"
            )));
        }
...
impl Storage for DirectoryPool {
    ...
    async fn copy_out(&self, volume: &str, path: &str) -> Result<u64> {
        self.copy_file_out(&self.volume_path(volume), volume, path)
            .await
    }

    /// From a snapshot, which is what a backup copies: the snapshot is a
    /// moment, and the live volume is not.
    async fn copy_out_snapshot(&self, snapshot: &str, path: &str) -> Result<u64> {
        self.copy_file_out(&self.snapshot_path(snapshot), snapshot, path)
            .await
    }
